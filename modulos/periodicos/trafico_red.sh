...
case "$1" in
	 ...
	 "procesar")
		...
		ACTUAL=$(ifconfig $(ifconfig | grep eth | awk '{print$1}') | grep -m 1 "TX" | awk '{print$2}' | cut -d ":" -f 2)
		
		REAL=$(echo $ACTUAL - $INICIO | bc)
		
		#comparo cantidad maxima de paquetes ip salientes con la cantidad actual.

		if [ $INICIO -ge $CANT_MAX_PAQUETESIP_SAL ];then
	...
	"informacion")
		...
		echo "La cantidad actual de paquetes ip salientes es : $REAL"
